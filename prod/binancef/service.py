...
def trade_off():
    ...
    b = datetime.now().replace(hour=8, minute=45, second=0, microsecond=0)
...
def _start():
    ...
    if trade_off():
        ...
        if gpid:
            ...
            send_wx_msg('进程组已停止运行[gpid={}]'.format(gpid))
        else:
        ...
        if not gpid:
            ...
            while True:
                ...
                if gpid:
                    ...
                    send_wx_msg('{}属于运行时间,成功启动服务[gpid={}]'.format(datetime.now(), gpid))
                    break
