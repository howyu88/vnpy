...
test_setting['name'] = 'triple_ma_test_{}'.format(datetime.now().strftime('%m%d_%H%M'))
...
test_setting['bar_interval_seconds'] = 120   # 回测数据的秒周期
...
strategy_setting = {
    f"triple_ma_{symbol}": {
        "class_name": "TurtleSignalStrategy_v3",
        "vt_symbol": f"{symbol}",
        "auto_init": True,
        "setting": {
            "backtesting": True,
